...
addpath osusvm/ %put your own path to osusvm here
...
train_set.pos   = '../../../Image_Datasets - Copy/Targets/train';
train_set.neg   = '../../../Image_Datasets - Copy/Distractors/train';
test_set.pos    = '../../../Image_Datasets - Copy/Targets/test';
test_set.neg    = '../../../Image_Datasets - Copy/Distractors/test';
...
% %%
% writematrix([XTrain' ytrain],'train.csv') 
% writematrix([XTest' ytest],'test.csv') 
%%
% Number of columns
numColumns = 1001;
...
for i = 1:numColumns - 1
    columnNames{i} = sprintf('Column %d', i);
end
columnNames{1001} = 'Y';
%%

% Step 3: Concatenate column names with the data matrix
matrixWithHeader = vertcat(columnNames, num2cell([XTrain' ytrain]));

% Step 4: Save matrixWithHeader as a .csv file
csvFileName = 'trainall.csv';
writecell(matrixWithHeader, csvFileName);
%%
matrixWithHeader = vertcat(columnNames, num2cell([XTest' ytest]));

% Step 4: Save matrixWithHeader as a .csv file
csvFileName = 'testall.csv';
writecell(matrixWithHeader, csvFileName);
